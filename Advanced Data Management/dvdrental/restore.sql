...
-- File paths need to be edited. Search for /Users/apple/advanced_data_management/dvdrental and
...
WITH
    TEMPLATE = template0 ENCODING = 'UTF8' LC_COLLATE = 'English_United States.1252' LC_CTYPE = 'English_United States.1252';
...
CREATE FUNCTION PUBLIC._GROUP_CONCAT(TEXT, TEXT) RETURNS 
TEXT LANGUAGE SQL IMMUTABLE AS $_$ 
	$_$
	SELECT
	    CASE
...
CREATE FUNCTION PUBLIC.FILM_IN_STOCK(P_FILM_ID INTEGER
, P_STORE_ID INTEGER, OUT P_FILM_COUNT INTEGER) RETURNS 
SETOF INTEGER LANGUAGE SQL AS $_$ 
	$_$
	SELECT inventory_id
	FROM inventory
	WHERE
...
CREATE FUNCTION PUBLIC.FILM_NOT_IN_STOCK(P_FILM_ID 
INTEGER, P_STORE_ID INTEGER, OUT P_FILM_COUNT INTEGER
...
CREATE FUNCTION PUBLIC.GET_CUSTOMER_BALANCE(P_CUSTOMER_ID 
INTEGER, P_EFFECTIVE_DATE TIMESTAMP WITHOUT TIME ZONE
...
CREATE FUNCTION PUBLIC.INVENTORY_HELD_BY_CUSTOMER(P_INVENTORY_ID 
...
ALTER FUNCTION
    public.inventory_held_by_customer(p_inventory_id integer) OWNER TO postgres;
...
CREATE FUNCTION PUBLIC.INVENTORY_IN_STOCK(P_INVENTORY_ID 
INTEGER) RETURNS BOOLEAN LANGUAGE PLPGSQL AS $$ 
...
ALTER FUNCTION
    public.inventory_in_stock(p_inventory_id integer) OWNER TO postgres;
...
CREATE FUNCTION PUBLIC.LAST_DAY(TIMESTAMP WITHOUT TIME 
ZONE) RETURNS DATE LANGUAGE SQL IMMUTABLE STRICT AS 
$_$ 
	$_$
	SELECT
	    CASE
	        ...
	        ELSE ( (
	                EXTRACT(
	                    ...
	                    FROM
	                        $1
	                ) operator(pg_catalog.||) '-' operator(pg_catalog.||) (
	                    EXTRACT(
	                        MONTH
	                        FROM
	                            $1
	                    ) + 1
	                ) operator(pg_catalog.||) '-01'
	            ):: date - INTERVAL '1 day'
	        ):: date
	    END $_$;
...
ALTER FUNCTION
    public.last_day(timestamp without time zone) OWNER TO postgres;
...
CREATE SEQUENCE public.customer_customer_id_seq START
WITH
...
CREATE FUNCTION PUBLIC.REWARDS_REPORT(MIN_MONTHLY_PURCHASES 
INTEGER, MIN_DOLLAR_AMOUNT_PURCHASED NUMERIC) RETURNS 
SETOF PUBLIC.CUSTOMER LANGUAGE PLPGSQL SECURITY DEFINER 
AS $_$ 
	$_$ DECLARE last_month_start DATE;
	last_month_end DATE;
	rr RECORD;
	tmpSQL TEXT;
	BEGIN 
...
CREATE SEQUENCE public.actor_actor_id_seq START
WITH
    1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
...
CREATE TABLE
    public.actor (
        actor_id integer DEFAULT nextval(
            'public.actor_actor_id_seq':: regclass
        ) NOT NULL,
        first_name character varying(45) NOT NULL,
        last_name character varying(45) NOT NULL,
...
CREATE SEQUENCE public.category_category_id_seq START
WITH
    1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
...
CREATE TABLE
    public.category (
        category_id integer DEFAULT nextval(
...
CREATE SEQUENCE public.film_film_id_seq START
WITH
    1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
...
CREATE TABLE
    public.film (
        film_id integer DEFAULT nextval(
            'public.film_film_id_seq':: regclass
        ) NOT NULL,
...
CREATE VIEW
    public.actor_info AS
SELECT
    a.actor_id,
    a.first_name,
    a.last_name,
    public.group_concat(
        DISTINCT ( ( (c.name):: text || ': ':: text) || (
                SELECT
                    public.group_concat( (f.title):: text) AS group_concat
                FROM ( (
                            public.film f
...
CREATE SEQUENCE public.address_address_id_seq START
WITH
    1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
...
CREATE TABLE
    public.address (
        address_id integer DEFAULT nextval(
...
CREATE SEQUENCE public.city_city_id_seq START
WITH
    1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
...
CREATE TABLE
    public.city (
        city_id integer DEFAULT nextval(
            'public.city_city_id_seq':: regclass
        ) NOT NULL,
...
CREATE SEQUENCE public.country_country_id_seq START
WITH
    1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
...
CREATE TABLE
    public.country (
        country_id integer DEFAULT nextval(
...
CREATE VIEW
    public.customer_list AS
SELECT
    cu.customer_id AS id, ( ( (cu.first_name):: text || ' ':: text
        ) || (cu.last_name):: text
    ) AS name,
    a.address,
...
SELECT
    ...
    film.length,
    film.rating,
    public.group_concat( ( ( (actor.first_name):: text || ' ':: text
            ) || (actor.last_name):: text
        )
    ) AS actors
FROM ( ( ( (
                    public.category
...
CREATE SEQUENCE public.inventory_inventory_id_seq START
WITH
    1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
...
CREATE SEQUENCE public.language_language_id_seq START
WITH
    1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
...
CREATE VIEW
    public.nicer_but_slower_film_list AS
SELECT
    film.film_id AS fid,
    film.title,
    film.description,
    ...
    public.group_concat( ( ( (
                    upper(
                        "substring"( (actor.first_name):: text, 1, 1)
                    ) || lower(
                        "substring"( (actor.first_name):: text, 2)
                    )
                ) || upper(
                    "substring"( (actor.last_name):: text, 1, 1)
                )
            ) || lower(
                "substring"( (actor.last_name):: text, 2)
            )
        )
    ) AS actors
FROM ( ( ( (
...
CREATE SEQUENCE public.payment_payment_id_seq START
WITH
    1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
...
CREATE TABLE
    public.payment (
        payment_id integer DEFAULT nextval(
...
CREATE SEQUENCE public.rental_rental_id_seq START
WITH
    1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
...
CREATE TABLE
    public.rental (
        rental_id integer DEFAULT nextval(
            'public.rental_rental_id_seq':: regclass
        ) NOT NULL,
...
CREATE VIEW
    public.sales_by_film_category AS
SELECT
    c.name AS category,
...
CREATE SEQUENCE public.staff_staff_id_seq START
WITH
    1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
...
CREATE TABLE
    public.staff (
        staff_id integer DEFAULT nextval(
            'public.staff_staff_id_seq':: regclass
        ) NOT NULL,
        first_name character varying(45) NOT NULL,
        last_name character varying(45) NOT NULL,
        address_id smallint NOT NULL,
...
CREATE SEQUENCE public.store_store_id_seq START
WITH
    1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
...
CREATE TABLE
    public.store (
        store_id integer DEFAULT nextval(
            'public.store_store_id_seq':: regclass
        ) NOT NULL,
...
CREATE VIEW
    public.sales_by_store AS
SELECT ( ( (c.city):: text || ',':: text) || (cy.country):: text
    ) AS store, ( ( (m.first_name):: text || ' ':: text
        ) || (m.last_name):: text
    ) AS manager,
    sum(p.amount) AS total_sales
FROM ( ( ( ( ( ( (
...
CREATE VIEW
    public.staff_list AS
SELECT
    s.staff_id AS id, ( ( (s.first_name):: text || ' ':: text
        ) || (s.last_name):: text
    ) AS name,
    a.address,
    a.postal_code AS "zip code",
...
\.
COPY public.actor (actor_id, first_name, last_name, last_update) FROM '/Users/apple/advanced_data_management/dvdrental/3057.dat';
...
\.
COPY public.address (address_id, address, address2, district, city_id, postal_code, phone, last_update) FROM '/Users/apple/advanced_data_management/dvdrental/3065.dat';
...
\.
COPY public.category (category_id, name, last_update) FROM '/Users/apple/advanced_data_management/dvdrental/3059.dat';
...
\.
COPY public.city (city_id, city, country_id, last_update) FROM '/Users/apple/advanced_data_management/dvdrental/3067.dat';
...
\.
COPY public.country (country_id, country, last_update) FROM '/Users/apple/advanced_data_management/dvdrental/3069.dat';
...
\.
COPY public.customer (customer_id, store_id, first_name, last_name, email, address_id, activebool, create_date, last_update, active) FROM '/Users/apple/advanced_data_management/dvdrental/3055.dat';
...
\.
COPY public.film (film_id, title, description, release_year, language_id, rental_duration, rental_rate, length, replacement_cost, rating, last_update, special_features, fulltext) FROM '/Users/apple/advanced_data_management/dvdrental/3061.dat';
...
\.
COPY public.film_actor (actor_id, film_id, last_update) FROM '/Users/apple/advanced_data_management/dvdrental/3062.dat';
...
\.
COPY public.film_category (film_id, category_id, last_update) FROM '/Users/apple/advanced_data_management/dvdrental/3063.dat';
...
\.
COPY public.inventory (inventory_id, film_id, store_id, last_update) FROM '/Users/apple/advanced_data_management/dvdrental/3071.dat';
...
\.
COPY public.language (language_id, name, last_update) FROM '/Users/apple/advanced_data_management/dvdrental/3073.dat';
...
\.
COPY public.payment (payment_id, customer_id, staff_id, rental_id, amount, payment_date) FROM '/Users/apple/advanced_data_management/dvdrental/3075.dat';
...
\.
COPY public.rental (rental_id, rental_date, inventory_id, customer_id, return_date, staff_id, last_update) FROM '/Users/apple/advanced_data_management/dvdrental/3077.dat';
...
\.
COPY public.staff (staff_id, first_name, last_name, address_id, email, store_id, active, username, password, last_update, picture) FROM '/Users/apple/advanced_data_management/dvdrental/3079.dat';
...
\.
COPY public.store (store_id, manager_staff_id, address_id, last_update) FROM '/Users/apple/advanced_data_management/dvdrental/3081.dat';
...
CREATE INDEX
    idx_actor_last_name ON public.actor USING btree (last_name);
...
CREATE INDEX
    idx_fk_address_id ON public.customer USING btree (address_id);
...
CREATE INDEX
    idx_fk_country_id ON public.city USING btree (country_id);
...
CREATE INDEX
    idx_fk_customer_id ON public.payment USING btree (customer_id);
...
CREATE INDEX
    idx_fk_film_id ON public.film_actor USING btree (film_id);
...
CREATE INDEX
    idx_fk_inventory_id ON public.rental USING btree (inventory_id);
...
CREATE INDEX
    idx_fk_language_id ON public.film USING btree (language_id);
...
CREATE INDEX
    idx_fk_rental_id ON public.payment USING btree (rental_id);
...
CREATE INDEX
    idx_fk_staff_id ON public.payment USING btree (staff_id);
...
CREATE INDEX
    idx_fk_store_id ON public.customer USING btree (store_id);
...
CREATE INDEX
    idx_last_name ON public.customer USING btree (last_name);
...
CREATE INDEX
    idx_store_id_film_id ON public.inventory USING btree (store_id, film_id);
...
CREATE TRIGGER FILM_FULLTEXT_TRIGGER 
	film_fulltext_trigger BEFORE
	INSERT OR
	UPDATE
	    ON public.film FOR EACH ROW
	EXECUTE
	    PROCEDURE tsvector_update_trigger(
	        'fulltext',
	        'pg_catalog.english',
	        'title',
	        'description'
...
CREATE TRIGGER LAST_UPDATED 
	last_updated BEFORE
	UPDATE
	    ON public.actor FOR EACH ROW
	EXECUTE
	    PROCEDURE public.last_updated();
...
CREATE TRIGGER LAST_UPDATED 
	last_updated BEFORE
	UPDATE
	    ON public.customer FOR EACH ROW
	EXECUTE
	    PROCEDURE public.last_updated();
...
CREATE TRIGGER LAST_UPDATED 
	last_updated BEFORE
	UPDATE
	    ON public.film_actor FOR EACH ROW
	EXECUTE
	    PROCEDURE public.last_updated();
...
CREATE TRIGGER LAST_UPDATED 
	last_updated BEFORE
	UPDATE
	    ON public.film_category FOR EACH ROW
	EXECUTE
	    PROCEDURE public.last_updated();
...
CREATE TRIGGER LAST_UPDATED 
	last_updated BEFORE
	UPDATE
	    ON public.inventory FOR EACH ROW
	EXECUTE
	    PROCEDURE public.last_updated();
...
CREATE TRIGGER LAST_UPDATED 
	last_updated BEFORE
	UPDATE
	    ON public.language FOR EACH ROW
	EXECUTE
	    PROCEDURE public.last_updated();
...
ALTER TABLE
    ONLY public.customer
ADD
    CONSTRAINT customer_address_id_fkey FOREIGN KEY (address_id) REFERENCES public.address(address_id) ON UPDATE CASCADE ON DELETE RESTRICT;
...
ALTER TABLE
    ONLY public.film_actor
ADD
    CONSTRAINT film_actor_actor_id_fkey FOREIGN KEY (actor_id) REFERENCES public.actor(actor_id) ON UPDATE CASCADE ON DELETE RESTRICT;
...
ALTER TABLE
    ONLY public.film_actor
ADD
    CONSTRAINT film_actor_film_id_fkey FOREIGN KEY (film_id) REFERENCES public.film(film_id) ON UPDATE CASCADE ON DELETE RESTRICT;
...
ALTER TABLE
    ONLY public.film_category
ADD
    CONSTRAINT film_category_category_id_fkey FOREIGN KEY (category_id) REFERENCES public.category(category_id) ON UPDATE CASCADE ON DELETE RESTRICT;
...
ALTER TABLE
    ONLY public.film_category
ADD
    CONSTRAINT film_category_film_id_fkey FOREIGN KEY (film_id) REFERENCES public.film(film_id) ON UPDATE CASCADE ON DELETE RESTRICT;
...
ALTER TABLE ONLY public.film
ADD
    CONSTRAINT film_language_id_fkey FOREIGN KEY (language_id) REFERENCES public.language(language_id) ON UPDATE CASCADE ON DELETE RESTRICT;
...
ALTER TABLE
    ONLY public.inventory
ADD
    CONSTRAINT inventory_film_id_fkey FOREIGN KEY (film_id) REFERENCES public.film(film_id) ON UPDATE CASCADE ON DELETE RESTRICT;
...
ALTER TABLE ONLY public.payment
ADD
    CONSTRAINT payment_customer_id_fkey FOREIGN KEY (customer_id) REFERENCES public.customer(customer_id) ON UPDATE CASCADE ON DELETE RESTRICT;
...
ALTER TABLE ONLY public.payment
ADD
    CONSTRAINT payment_rental_id_fkey FOREIGN KEY (rental_id) REFERENCES public.rental(rental_id) ON UPDATE CASCADE ON DELETE
SET NULL;
...
ALTER TABLE ONLY public.payment
ADD
    CONSTRAINT payment_staff_id_fkey FOREIGN KEY (staff_id) REFERENCES public.staff(staff_id) ON UPDATE CASCADE ON DELETE RESTRICT;
...
ALTER TABLE ONLY public.rental
ADD
    CONSTRAINT rental_customer_id_fkey FOREIGN KEY (customer_id) REFERENCES public.customer(customer_id) ON UPDATE CASCADE ON DELETE RESTRICT;
...
ALTER TABLE ONLY public.rental
ADD
    CONSTRAINT rental_inventory_id_fkey FOREIGN KEY (inventory_id) REFERENCES public.inventory(inventory_id) ON UPDATE CASCADE ON DELETE RESTRICT;
...
ALTER TABLE ONLY public.store
ADD
    CONSTRAINT store_address_id_fkey FOREIGN KEY (address_id) REFERENCES public.address(address_id) ON UPDATE CASCADE ON DELETE RESTRICT;
...
ALTER TABLE ONLY public.store
ADD
    CONSTRAINT store_manager_staff_id_fkey FOREIGN KEY (manager_staff_id) REFERENCES public.staff(staff_id) ON UPDATE CASCADE ON DELETE RESTRICT;
